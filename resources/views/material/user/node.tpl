																		...
																		{foreach $node_muport as $single_muport}
																			...
																			<div class="card">
																				<div class="card-main">
																					<div class="card-inner">
																					<p class="card-heading" >
																						<a href="javascript:void(0);" onClick="urlChange('{$node->id}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$prefix} {if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - {$single_muport['server']->server} 单端口多用户</a>
																						<span class="label label-brand-accent">{$node->status}</span>
																					</p>
